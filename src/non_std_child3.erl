...
start_link() ->
	lager:emergency("******************* non_std_child3: START_LINK~n", []),
    gen_server:start_link({local, ?MODULE}, ?MODULE, [], []).

init([]) ->
	lager:emergency("******************* non_std_child3: INIT~n", []),
    {ok, []}.
...
handle_call(ping, _From, State) ->
    lager:emergency("******************* non_std_child3: PING~n", []),
    {reply, {ok, pong}, State};

handle_call(crash, _From, State) ->
    lager:emergency("******************* non_std_child3: CRASH!!!~n", []),
	X=1,
    {reply, X=2, State};
...
terminate(_Reason, _State) ->
	lager:emergency("******************* non_std_child3: TERMINATE~n", []),
    ok.
